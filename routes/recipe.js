...
module.exports.registerRoutes = function(models, multiparty, utils){
  ...
  router.put('/:id/comment/:userid', function(req, res, next){
      var date = Date.now();
  ...
  router.put('/:id/toggle-like/:userid', function(req, res, next){
    var liked = false;
    models.Recipe.findOne({_id: req.params.id}, function(err, recipe){
      ...
      if(utils.contains(recipe.likes, req.params.userid)){
        utils.remove(recipe.likes, req.params.userid);
        liked = false;
      } else {
